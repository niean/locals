...
- 左侧工具栏（c-toolbar）：裁剪、画笔、文字、马赛克、形状五种编辑工具切换按钮
- 顶部属性栏（c-property-bar）：工具参数选项（根据当前工具动态显示）、逆时针旋转按钮、撤销/重做按钮、打开图片按钮、导出格式选择、下载按钮
- 中间画布区域（o-app__main）：图片编辑主区域，包含画布和占位提示
